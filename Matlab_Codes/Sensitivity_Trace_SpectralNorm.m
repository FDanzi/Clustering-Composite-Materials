...
for f = 1:size(fibers,1)
    ...
    for m = 1:size(matrices,1)
        ...
        for Vf = Vf_range
            ...
            %% Compute Baseline Q* Matrix and Its Spectral Norm
            Q_base = compute_Qstar(pi1, pi2, pi3);
            spec_norm_base = norm(Q_base, 2); % Spectral Norm

            %% Compute Sensitivities Using Spectral Norm
            spec_norm_pi1 = norm(compute_Qstar(pi1 + dPi, pi2, pi3), 2);
            spec_norm_pi2 = norm(compute_Qstar(pi1, pi2 + dPi, pi3), 2);
            spec_norm_pi3 = norm(compute_Qstar(pi1, pi2, pi3 + dPi), 2);

            dSpec_pi1 = abs(spec_norm_pi1 - spec_norm_base) / dPi;
            dSpec_pi2 = abs(spec_norm_pi2 - spec_norm_base) / dPi;
            dSpec_pi3 = abs(spec_norm_pi3 - spec_norm_base) / dPi;

            %% Identify Most Influential π-Group
            [~, maxIndex] = max([dSpec_pi1, dSpec_pi2, dSpec_pi3]);
            pi_labels = ["E2/E1", "G12/E1", "ν12"];
            dominant_pi = pi_labels(maxIndex);

            %% Store Results
            results = [results; {fiber_name, matrix_name, Vf, pi1, pi2, pi3, dSpec_pi1, dSpec_pi2, dSpec_pi3, dominant_pi}];
        end
    end
end
...
%% Save Table to CSV
writetable(results_table, 'spectral_norm_sensitivity_results.csv');
