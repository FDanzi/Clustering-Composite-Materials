...
%     with respect to the Eigenvalue Norm.
%
%  License:
...
for f = 1:size(fibers,1)
    ...
    for m = 1:size(matrices,1)
        ...
        for Vf = Vf_range
            ...
            %% Compute Baseline Q* Matrix and Its Eigenvalues
            Q_base = compute_Qstar(pi1, pi2, pi3);
            lambda_base = eig(Q_base);

            %% Compute Sensitivities Using Eigenvalue Norm
            lambda_pi1 = eig(compute_Qstar(pi1 + dPi, pi2, pi3));
            lambda_pi2 = eig(compute_Qstar(pi1, pi2 + dPi, pi3));
            lambda_pi3 = eig(compute_Qstar(pi1, pi2, pi3 + dPi));

            dLambda_pi1 = norm(lambda_pi1 - lambda_base);
            dLambda_pi2 = norm(lambda_pi2 - lambda_base);
            dLambda_pi3 = norm(lambda_pi3 - lambda_base);

            %% Identify Most Influential π-Group
            [~, maxIndex] = max([dLambda_pi1, dLambda_pi2, dLambda_pi3]);
            pi_labels = ["E2/E1", "G12/E1", "ν12"];
            dominant_pi = pi_labels(maxIndex);

            %% Store Results
            results = [results; {fiber_name, matrix_name, Vf, pi1, pi2, pi3, dLambda_pi1, dLambda_pi2, dLambda_pi3, dominant_pi}];
        end
    end
end
...
%% Save Table to CSV
writetable(results_table, 'eigenvalue_sensitivity_results.csv');
